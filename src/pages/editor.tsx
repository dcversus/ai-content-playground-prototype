...
import { addEdge, Edge, useEdgesState, useNodesState, Node } from '@xyflow/react'
  ...
  const richNodes = nodes.map((node) => ({
    ...
    data: {
      ...
      onEdit: (nodeId: string, data: any) => {
        ...
        setNodes((nds) => nds.map((n) => n.id === nodeId ? { ...n, data: merge(n.data, data) } : n))
      },
    },
  }))
  ...
  return (
    <ReactFlowProvider>
      <div className="flex h-screen">
                ...
                {nodes.map((node) => {
                  ...
                  return (
                    <SelectItem key={node.id} value={node.id} className="flex items-center gap-2">
                      <span className="inline-flex gap-2 items-center"><TypeIcon className="h-4 w-4" /> {node.data.label as string}</span>
                    </SelectItem>
                  )
                })}
              </SelectContent>
        ...
        <MiniMap position="bottom-left" pannable zoomable nodeColor="#e2e2e2" maskColor="rgb(240, 240, 240, 0.6)" style={{ width: 320, height: 240, margin: 0 }}/>
      </aside>
